...
| 思路                           | 过程                             |
| ------------------------------ | -------------------------------- |
| ![](./img/构造二叉树-demo.png) | ![](./img/构造二叉树-demo-2.png) |
...
<LeetCodeLink url="https://leetcode.cn/problems/lowest-common-ancestor-of-a-binary-tree/description/" />
...
如何判断一个节点是节点 q 和节点 p 的公共祖先呢？
情况 1：如果当前节点是 q 和 p 的公共祖先，那么当前节点的左右子树中一定包含 q 和 p。
1、首先最容易想到的一个情况：如果找到一个节点，发现左子树出现结点 p，右子树出现节点 q，或者 左子树出现结点 q，右子树出现节点 p，那么该节点就是节点 p 和 q 的最近公共祖先。
2、判断逻辑是：如果递归遍历遇到 q，就将 q 返回，遇到 p 就将 p 返回，那么如果 左右子树的返回值都不为空，说明此时的中节点，一定是 q 和 p 的最近祖先。

情况 2：祖先是节点本身 p(q)，它拥有一个子孙节点 q(p)
其实情况一 和 情况二 代码实现过程都是一样的，也可以说，实现情况一的逻辑，顺便包含了情况二。
因为遇到 q 或者 p 就返回，这样也包含了 q 或者 p 本身就是 公共祖先的情况。

可以采用递归实现：
1、确定递归函数的参数和返回值：`function find(root, p, q) {}`，`return type: TreeNode`
2、确定终止条件：
当空节点直接 return：`if (!root) return null`
当 p 或者 q 找到，返回该节点：`if (root === p || root === q) return root`（这个判断也是包含情况 2 的判断）
3、确定单层递归逻辑：
后序：左右中，左逻辑`left = find(root.left, p, q)`，右逻辑`right = find(root.right, p, q)`，
中逻辑`if (left !== null && right !== null) return root`说明找到，返回该节点，
`if (left === null && right !== null) return right`，`if (left !== null && right === null) return left`（某侧为空，将另一侧继续向上返回，回溯过程）
```
...
<LeetCodeLink url="https://leetcode.cn/problems/lowest-common-ancestor-of-a-binary-search-tree/description/" />
...
```md [思路]
这道题如果用上一题的代码也是可以正常通过的，就是把二叉搜索树当成普通树看，
这里说一下其他的思路写法，利用二叉搜索树来做。

本题是二叉搜索树，二叉搜索树是有序的，那得好好利用一下这个特点。
在有序树里，如果判断一个节点的左子树里有 p，右子树里有 q 呢？
因为是有序树，所以 如果 中间节点是 q 和 p 的公共祖先，那么 中节点的数组 一定是在 [p, q]区间的。即 中节点 > p && 中节点 < q 或者 中节点 > q && 中节点 < p。（依赖于二叉搜索树的特性）

**写题整体思路**：
如果当前节点的值比搜索值大，往左搜索
如果当前节点的值比搜索值小 往右搜索
如果以上两种情况都不是 说明当前节点的值就在中间

这道题就可以用递归或者迭代法了，两种方法都比较简单。
```
...
var lowestCommonAncestor = function (root, p, q) {
  if (!root) return null
  // 如果 p 和 q 都小于当前节点，则去左子树找
  if (root.val > p.val && root.val > q.val) {
    let left = lowestCommonAncestor(root.left, p, q)
    // 返回左子树的查找结果
    return left
  }
  // 如果 p 和 q 都大于当前节点，则去右子树找
  if (root.val < p.val && root.val < q.val) {
    let right = lowestCommonAncestor(root.right, p, q)
    // 返回右子树的查找结果
    return right
  }
  // 如果 p 和 q 在当前节点的两侧，或者其中一个就是当前节点，则当前节点就是最近公共祖先
  return root
}
```
