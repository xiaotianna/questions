...
运行函数后，可以从图中看到内存是一直飚升的，在`内存`面板点击刷子进行清空，清理不掉，因为控制台函数的引用，导致内存泄漏。

所以需要清空控制台，就可以点击刷子进行清空了。
...
计算机内存储的 0.1 是一个近似值，但是 0.1+0.1 = 0.2 实际也是不相等的，只是前几位是一样的。
...
如果是例如百度这样的，主域名是一致的可以完全利用cookie解决，例如：fanyi.baidu.com和map.baidu.com天然可以共享身份。

下面说的是两个不同域名的网站，例如abc.com与123.com只登录一个网站，其他一个网站也保持登录状态。
...
- 用户访问系统 B，发现你没登陆，也跳转到认证中心

> `b.com` -> `https://sso.com/login?redirect=b.com`

- 浏览器访问 sso.com 时，自动带上 sso.com 的 cookie
- 认证中心判断：你已经登录过了，再跳回 B（这个时候的用户信息不回返回，为了安全会将临时凭证ticket放入url中）

> `https://sso.com/login?redirect=b.com` -> `https://b.com/callback?ticket=abc123`

- B 拿 ticket 去认证中心“换用户信息”，B 后端会调用认证中心接口
...
## 问题 61：编译型语言和解释型语言有什么区别？能否举几个例子

编译型语言和解释型语言的核心区别在于**代码执行前是否需要完全翻译成机器码**，以及**执行方式**的不同：

**1. 编译型语言**

- **执行流程**：源代码通过**编译器**一次性翻译成机器码（二进制文件），之后直接运行机器码，无需再次编译。
- **特点**：
  - 执行速度快（直接运行机器码）
  - 跨平台性差（不同系统需要重新编译）
  - 开发周期中多了编译步骤
- **例子**：
  - C/C++：编译后生成.exe 等可执行文件
  - Java（特殊：先编译为字节码，再由 JVM 解释执行，兼具两者特点）
  - Go：编译为特定平台的机器码，可直接运行

**2. 解释型语言**

- **执行流程**：源代码不需要提前编译，而是由**解释器**逐行实时翻译成机器码并执行。
- **特点**：
  - 执行速度较慢（每次运行都需要解释）
  - 跨平台性好（只需对应平台的解释器）
  - 开发效率高（写完即可运行，无需编译）
- **例子**：
  - Python：通过 Python 解释器逐行执行
  - JavaScript：在浏览器或 Node.js 环境中实时解释执行

## 问题 62：为什么 JS 不编译也可以执行？

在面试中可以这样回答：

JavaScript 之所以不需要像 Java、C# 等语言那样显式编译就能执行，核心原因在于它是一种**解释型语言**（现代实践中是解释与编译结合），其执行过程与编译型语言有本质区别：

1. **执行方式不同**  
   编译型语言需要通过编译器将源代码一次性转换为机器码（二进制文件），之后才能运行；而 JavaScript 由**解释器**逐行读取代码，边解析边执行，不需要提前生成独立的可执行文件。

2. **依赖宿主环境的解释器**  
   JavaScript 的执行依赖宿主环境（如浏览器、Node.js）内置的解释器（如 V8 引擎）。当代码加载后，解释器会直接处理并执行，省去了用户手动编译的步骤。

3. **设计目标决定**  
   JavaScript 诞生之初是为了在浏览器中实现简单交互，设计上追求**快速开发和即时执行**，无需编译步骤能降低使用门槛，更符合网页脚本的场景需求。

## 问题 63：为什么表单提交不会出现跨域

同源策略主要限制 JavaScript 发起的请求（如 XMLHttpRequest、Fetch 等），防止恶意脚本通过浏览器获取其他域的敏感数据，而表单提交（`<form>` 标签）是浏览器原生行为，非 JavaScript 发起的请求，因此不受同源策略限制，类似于`<script>`标签一样，不受跨域影响

## 问题 64：ES Module 的 import 模块的加载机制

#### 1、只读的静态加载

- 只读：模块导入的所有成员（变量、函数、类、对象），均为只读引用，不允许在导入方修改导入的成员
  - 如果导入的是基本类型：导入方赋值修改会直接报错
  - 如果导入的是引用类型：可以修改对象的属性 / 数组的元素（因为引用地址不变），但不能给这个引用变量重新赋值
  ...
  num = 20 // 报错：Assignment to constant variable (只读，禁止修改)
  obj.name = 'ES Module' // 允许，修改的是引用指向的内容，引用地址不变
  obj = { age: 20 } // 报错：只读，禁止重新赋值引用
  ```

  :::

- 静态解析+静态绑定：
  - 导入的成员是动态响应导出方的变化的：导出方修改了变量的值，导入方会实时同步这个新值
  - 编译阶段就确定所有依赖，能做静态优化（tree-shaking 摇树优化的核心原理）
  ...
  console.log(count) // 0
  add() // 调用导出方的函数，修改导出方的count
  console.log(count) // 1 实时同步导出方的最新值，这就是「静态绑定」
  ```
...
ESM 对同一个模块标识符的模块，遵循**单例加载**。也就是说无论同一个模块被导入多少次，这个模块只会被加载、实例化、执行一次，后续所有的`import`语句，都不会重新加载模块，而是直接复用第一次加载后生成的「模块实例」和绑定关系。

> 目的：避免重复加载和执行模块代码，节省内存和性能开销

::: details 示例

```js
// counter.js
console.log('counter模块被执行了')
...
运行结果：只会执行一次 counter.js 的 log 输出

:::

但是在单例模式下可能会导致内存泄漏的问题，可以通过使用 `WeakMap` 来解决

#### 3、ESM 模块的顶级作用域特性

ESM 模块天生拥有 **独立的顶级模块作用域**，这是和 CommonJS 以及「全局脚本」的核心区别之一，也是高频考点：

1. ESM 模块的**所有顶级变量、函数、类、对象**，都是「模块私有」的，**不会挂载到全局对象**（浏览器中是`window`，Node.js 中是`global`）；
2. 不同模块的顶级变量名可以重名，互不冲突，因为作用域隔离；
3. 如果想让模块的成员暴露到全局，需要手动挂载（不推荐）：`window.xxx = xxx`。
...
// 情况1：ESM模块 (xxx.js)
const a = 10 // 顶级变量
console.log(window.a) // undefined ❌ 不会挂载到全局

// 情况2：普通全局脚本 (通过<script>引入，无type="module")
const a = 10
console.log(window.a) // 10 ✅ 挂载到全局
```
...
ES Module 加载任意一个模块，都会遵循**严格的 3 阶段执行流程**，**先解析、再实例化、最后执行**，**所有阶段都是单线程同步执行**，且整个流程中模块只会被加载一次，不会重复加载。

> 补充：这个流程对「入口模块」和「依赖模块」完全一致，入口模块加载时，会递归加载所有依赖的子模块，所有子模块都完成这 3 个阶段后，入口模块才会完成执行。

- 阶段 1：解析。核心工作：**解析模块的引用地址，确定要加载的真实模块文件，生成「模块记录」**

  ::: details
  1. 解析`import xxx from '模块地址'`中的「模块地址」，区分三种模块类型：
     - 相对路径模块：`./a.js`、`../utils/b.js`（必须写完整后缀，如`.js`，ESM 不支持省略）；
     - 绝对路径模块：`/src/c.js`；
     - 裸模块：`vue`、`axios`（npm 安装的第三方包，解析`node_modules`中的文件）；
  2. 处理模块地址的规范化（去重、补全路径），生成唯一的「模块标识符」；
  3. 核心规则：**此阶段完全不执行任何代码**，只做地址解析和依赖收集。

  :::

- 阶段 2：实例化 —— 建内存，存模块成员。核心工作：**为所有模块分配内存空间，创建模块的「模块环境记录」，建立导入和导出的绑定关系**，这是 ESM 「静态绑定」的核心实现阶段，也是最关键的一步。

  ::: details
  1. 为每个模块创建独立的**模块作用域**（ESM 模块天生拥有独立作用域，顶级变量/函数不会挂载到全局`window/global`）；
  2. 在内存中为模块的「导出成员」开辟空间（只占位，**不赋值**）；
  3. **核心动作：建立「导入-导出的绑定桥」** —— 导入方的变量名，会直接关联到导出方的内存地址，二者指向同一个内存空间，这就是「只读引用」和「实时同步」的底层原因；
  4. 此阶段的关键特征：**只分配内存、只绑定关系，不执行代码，不赋值**，所有变量都是「未初始化状态」。

  :::

- 阶段 3：执行 —— 跑代码，赋真实值。核心工作：**执行模块的代码，为实例化阶段分配的内存空间赋值，执行模块的顶级代码**，是整个加载流程的最后一步。

  ::: details
  1. 按照「深度优先」的顺序执行模块代码（先执行最底层的依赖模块，再执行上层模块，最后执行入口模块）；
  2. 执行模块的**顶级代码**（变量赋值、函数定义、顶级`if`逻辑等），为实例化阶段的内存空间赋上真实值；
  3. 函数/类的定义会被提升，但赋值逻辑按代码顺序执行；
  4. 核心规则：**一旦模块执行完成，其导出成员的绑定关系永久生效**，后续导入方永远能拿到最新值。

  :::

## 问题 65：== 和 === 的区别？

- `===` 严格相等（全等）：`===` 会同时比较**值和数据类型**，只有当两者完全一致时，结果才为 true，不会进行任何类型转换。如果类型不一致，直接返回 false。
- `==` 宽松相等（相等）：`==` 会先尝试类型转换，将不同类型的值转换为同一类型后，再比较值是否相等。规则：会调用 toString() 或 valueOf()，例如：`3 == {}`，`{}` 转换为 `"[object Object]"`（字符串），再与 3（number）比较，字符串 "[object Object]" 无法转为有效的数字，最终比较 3 == NaN，结果为 false。

## 问题 66：定时任务（setTimeout/setInterval）用了一定要销毁吗？

- 一次性任务（如 setTimeout 5s 后执行）：执行完毕后会自动清理，无需手动销毁。
- 重复任务（如 setInterval）：必须用 clearInterval 销毁，否则会一直执行。
- 组件销毁前（如 onUnmounted）：无论哪种定时任务，若未执行完毕，都需手动清除（避免组件已销毁但任务仍在运行，导致内存泄漏）。

## 问题 67：sse vs ws

AI 回答天生就是「单向流式输出」，用户只需要发送一次提问，然后等待服务器逐字返回答案。这个过程不需要客户端再发消息，完全符合 SSE 的单向模型。

WS 的双向能力在这里完全用不上，属于功能过剩，所以说比较重。

- 单向流式正好匹配

AI 生成是服务器push，客户端只接收，不需要客户端主动发消息给服务器

- 简单易用
  - SSE 基于标准 HTTP，前端只需 EventSource
  - WebSocket 需要握手、心跳、重连逻辑

- 协议与握手更重
  - SSE：基于 HTTP/1.1 / HTTP/2，复用普通连接，握手就是一次 GET 请求
  - WebSocket：需要 HTTP 升级握手（Upgrade: websocket），额外握手开销，独立协议栈

## 问题 66：for vs forEach谁的执行效率高

结论：`for` 循环比 `forEach` 更快

- for：for 是语言内建结构，没有额外函数调用；执行路径更短，开销更低
- forEach：有额外开销，每次循环都会调用一次回调函数，存在函数调用成本 + 作用域处理
...
> ```js
> arr.forEach((item) => {
>   if (item === 5) return // 只是跳过当前，不是 break
> })
> ```
>
> 2. forEach中不能写 async
