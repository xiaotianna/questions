...
- 第一组：
  ...
  - 响应头设置：`Last-Modified(时间戳)`，表示资源最后修改时间

  > 初次加载时，服务器返回一个时间

  - 请求头设置：`If-Modified-Since`

  > 第一次响应后，客户端发送请求时，会在请求头携带 `If-Modified-Since`，值为 `Last-Modified` 的时间戳，后端去判断最后修改时间是否变化，变了就返回最新数据，没变就返回状态码 `304`，走缓存
