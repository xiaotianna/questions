...
- chunk：Webpack 打包过程中生成的中间文件块，是模块拆分后的产物（比如代码分割、懒加载拆分出的文件），还未经过最终优化处理。

- bundle：Webpack 打包的最终输出文件，由一个或多个 chunk 合并、优化（压缩、Tree-shaking 等）后生成，直接用于浏览器运行。

简单类比：chunk 是“打包中的半成品”，bundle 是“打包好的成品”。
