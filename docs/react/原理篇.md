...
- **Render 阶段**：此阶段会进行 Fiber 树的构建（包括新 Fiber 树的创建和与旧 Fiber 树的对比，即协调过程），同时标记出需要执行的副作用（如 DOM 操作、生命周期方法调用等）。由于该阶段的操作不直接涉及 DOM 修改，这个阶段是可以被中断的。

> ⚠️ 注意：**Render 阶段包含协调（Reconciliation）过程**，但不能简单说 Render 阶段等同于协调阶段。
