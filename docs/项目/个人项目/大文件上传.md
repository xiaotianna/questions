...
- 原理
  ...
  - 发请求通知服务器进行分片合并
- 实现步骤
  - FileReader 读取文件的内容，计算文件 hash
  - 通过 FormData 上传分片，并添加文件 hash，索引等标识
  - 服务端接收到分片，并保存到临时目录
...
```js
// 计算合理的线程数
async function getIdealThreadCount() {
  const maxBrowserWorkers = 20 // 浏览器最大限制（如20）
  const cpuCores = navigator.hardwareConcurrency || 4 // CPU核心数（默认4）
  // 决策逻辑：取「CPU核心数、浏览器最大限制、8」的最小值，且至少1个
  const idealCount = Math.min(cpuCores, maxBrowserWorkers, 8)
  return Math.max(idealCount, 1) // 确保至少1个线程
}

// 创建线程
for (let threadId = 0; threadId < threadCount; threadId++) {
  // 计算当前线程处理的切片范围（startIndex ~ endIndex）
  const startIndex = threadId * chunksPerThread
  const endIndex = Math.min(startIndex + chunksPerThread, totalChunks)
  const threadChunks = fileChunks.slice(startIndex, endIndex) // 当前线程的任务切片

  // 创建Worker
  const worker = new Worker('./md5-worker.js')

  // 向Worker发送任务：当前线程的切片、线程ID
  worker.postMessage({
    threadId,
    chunks: threadChunks // 传递切片（Blob格式，结构化克隆算法支持）
  })
}
```
...
## 为什么大文件 MD5 要放 Web Worker

- MD5 计算是 CPU 密集型操作，大文件会跑很久。
- JS 是单线程，主线程忙着算 MD5，页面会卡死、点不动、卡顿。
- Web Worker 是后台独立线程，不阻塞主线程，页面依然流畅。

> cpu密集：一直在计算、运算、处理逻辑，例如：大文件计算 MD5
>
> io密集：一直在等外部资源，不怎么算，例如：读写文件
