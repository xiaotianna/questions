...
起因：在学 `next.js` 的时候，发现很多的框架都使用约定式路由，所以尝试开发一个插件，将 `next.js` 的路由规则，转换为 `vue-router` 的路由规则。

---

思路：

1. 读取文件目录，递归生成路由表，写入虚拟模块中
2. 根据 `babel`，将 `main.ts` 中的代码转成 `ast`，再输出到 `main.ts` 中

> `vite` 虚拟模块：[https://cn.vite.dev/guide/api-plugin.html#virtual-modules-conventiontext](https://cn.vite.dev/guide/api-plugin.html#virtual-modules-convention)
